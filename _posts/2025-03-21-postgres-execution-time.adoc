...
<3> A https://www.postgresql.org/docs/current/parallel-plans.html#PARALLEL-SCANS[parallel sequential scan^] on 30 million rows is a major bottleneck.
...
----
<1> The number of workers varies depending on the available CPU cores and the PostgreSQL configuration.
<2> A https://www.postgresql.org/docs/current/parallel-plans.html#PARALLEL-SCANS[parallel sequential scan^] on 30 million rows is a major bottleneck.
...
You can also check the number of dead tuples by querying the https://www.postgresql.org/docs/current/monitoring-stats.html#MONITORING-PG-STAT-ALL-TABLES-VIEW[pg_stat_user_tables^] view:
